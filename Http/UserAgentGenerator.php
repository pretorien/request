...
class UserAgentGenerator
{
    ...
    private static function arrayRandom($array)
    {
    ...
    {
        return self::arrayRandom(empty($lang) ? self::$languages : $lang);
    }

    private static function getProcessor($os)
    {
        return self::arrayRandom(self::$processors[$os]);
    }

    private static function getVersionNt()
    {
        // Win2k (5.0) to Win 7 (6.1).
        return mt_rand(5, 6) . '.' . mt_rand(0, 1);
    }

    private static function getVersionOsx()
    {
        return '10_' . mt_rand(5, 7) . '_' . mt_rand(0, 9);
    }

    private static function getVersionWebkit()
    {
        return mt_rand(531, 536) . mt_rand(0, 2);
    }

    private static function getVersionChrome()
    {
        return mt_rand(13, 15) . '.0.' . mt_rand(800, 899) . '.0';
    }

    private static function getVersionGecko()
    {
        return mt_rand(17, 31) . '.0';
    }

    private static function getVersionIe()
    {
        return '9.0';
    }

    private static function getVersionTrident()
    {
        return '7.0';
    }

    private static function getVersionNet()
    {
        ...
        return self::arrayRandom($frameworks) . $rev;
    }

    private static function getVersionSafari()
    {
        if (mt_rand(0, 1) == 0) {
    ...
    private static function getVersionOpera()
    {
    ...
    {
        $version = ' Version/' . self::getVersionSafari();
    ...
    {
        $nt = self::getVersionNt();
        $ie = self::getVersionIe();
        $trident = self::getVersionTrident();
        $net = self::getVersionNet();
    ...
    public static function random($browser = null, $os = 'win', $lang = ['fr-FR'])
    {
        if (is_null($browser)) {
